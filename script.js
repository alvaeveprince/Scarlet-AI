...
function loadChat(id) {
  ...
  chat.messages.forEach(m => {
    if (m.role !== 'system') App.conversationHistory.push({ role: m.role, parts: [{ text: m.text || '' }] });
  });
...
async function callGemini(userText, imageData = null) {
  ...
  // Build contents (history + current)
  let contents = [];
  if (App.settings.memoryEnabled && App.conversationHistory.length > 1) {
    // Include all but the last (which is the current message we're about to send)
    contents = App.conversationHistory.slice(-6).map(m => ({
...
if (!resp.ok) {
  ...
  if (resp.status === 401) throw new Error('Invalid API key — check OpenRouter key');
